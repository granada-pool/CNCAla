...
const print = dictMonadState => {
  ...
  const for_ = Data$dFoldable.for_(Applicative0)(Data$dFoldable.foldableMaybe);
  const get1 = dictMonadState.state(s => Data$dTuple.$Tuple(s, s));
  return dictMonadWriter => path => a => Bind1.bind(for_(Test$dSpec$dTree.parentSuite(path))(suite => Bind1.bind(get1)(s => {
    if (s.lastPrintedSuitePath.tag === "Just" && eq(s.lastPrintedSuitePath._1)(suite.path)) { return Applicative0.pure(); }
    return Bind1.bind(dictMonadWriter.MonadTell1().tell(Test$dSpec$dStyle.styled([...Test$dSpec$dStyle.bold, ...Test$dSpec$dStyle.magenta])(intercalate(" » ")([
      ...Data$dArray.mapMaybe(x => x.name)(suite.path),
      suite.name
    ])) + "\n"))(() => {
      const $0 = {...s, lastPrintedSuitePath: Data$dMaybe.$Maybe("Just", suite.path)};
...
const consoleReporter = /* #__PURE__ */ Test$dSpec$dReporter$dBase.defaultReporter(initialState)(/* #__PURE__ */ Test$dSpec$dReporter$dBase.defaultUpdate({
  ...
  putRunningItems: b => a => ({runningItems: b, lastPrintedSuitePath: a.lastPrintedSuitePath}),

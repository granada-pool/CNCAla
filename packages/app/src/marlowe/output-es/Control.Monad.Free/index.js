...
const freeBind = {bind: v => k => $Free(v._1, Data$dCatList.snoc(v._2)(k)), Apply0: () => freeApply};
const freeApply = {
  apply: f => a => $Free(f._1, Data$dCatList.snoc(f._2)(f$p => $Free(a._1, Data$dCatList.snoc(a._2)(a$p => freeApplicative.pure(f$p(a$p)))))),
...
const semigroupFree = dictSemigroup => (
  {append: a => b => freeApply.apply($Free(a._1, Data$dCatList.snoc(a._2)(x => $Free($FreeView("Return", dictSemigroup.append(x)), Data$dCatList.CatNil))))(b)}
);
const freeMonadRec = {
  tailRecM: k => a => {
    const $0 = k(a);
...
const substFree = k => {
  const go = f => {
    ...
    if (v.tag === "Bind") {
      const $0 = k(v._1);
      return $Free($0._1, Data$dCatList.snoc($0._2)(x => go(v._2(x))));
    }
    $runtime.fail();
  };
  return go;
        ...
        const go = x => {
          ...
          if ($1.tag === "Left") {
            return $0.map(x$1 => $Free(
              ...
              Data$dCatList.snoc(Data$dCatList.CatNil)(Control$dBind.identity)
            ))(traverse1(go)($1._1));
          }

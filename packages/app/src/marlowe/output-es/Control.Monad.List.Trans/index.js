...
const tail = dictMonad => {
  ...
  return l => dictMonad.Bind1().Apply0().Functor0().map(Data$dMaybe.functorMaybe.map(Data$dTuple.snd))(uncons1(l));
};
...
const head = dictMonad => {
  const uncons1 = uncons(dictMonad);
  return l => dictMonad.Bind1().Apply0().Functor0().map(Data$dMaybe.functorMaybe.map(Data$dTuple.fst))(uncons1(l));
};

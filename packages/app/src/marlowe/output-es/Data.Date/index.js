...
const greaterThan = /* #__PURE__ */ (() => {
  ...
  return a1 => a2 => $0.compare(a1)(a2) === "GT";
})();
...
const weekday = v => {
  ...
  if (n === 0) {
    const $0 = Data$dDate$dComponent.boundedEnumWeekday.toEnum(7);
    if ($0.tag === "Just") { return $0._1; }
    $runtime.fail();
  }
  const $0 = Data$dDate$dComponent.boundedEnumWeekday.toEnum(n);
  if ($0.tag === "Just") { return $0._1; }
  $runtime.fail();
};
const showDate = {
...
const enumDate = {
  succ: v => {
    ...
    const sd = greaterThan(v1)(Data$dMaybe.$Maybe("Just", lastDayOfMonth(v._1)(v._2))) ? Data$dMaybe.Nothing : v1;
    return Data$dMaybe.applyMaybe.apply(Data$dMaybe.applyMaybe.apply((() => {
      if (
        ...
        const $1 = v._1 + 1 | 0;
        if ($1 >= -271820 && $1 <= 275759) { return Data$dMaybe.$Maybe("Just", $$Date($1)); }
      ...
      ? Data$dMaybe.$Maybe("Just", 1)
      : sd);
  },
  pred: v => {
    const pm = Data$dDate$dComponent.enumMonth.pred(v._2);
    const $0 = v._3 - 1 | 0;
    const pd = $0 >= 1 && $0 <= 31 ? Data$dMaybe.$Maybe("Just", $0) : Data$dMaybe.Nothing;
    const m$p = (() => {
      if (
        (() => {
...
const canonicalDate = y => m => d => canonicalDateImpl(
  y$p => m$p => d$p => $$$Date(
    y$p,
    (() => {
      const $0 = Data$dDate$dComponent.boundedEnumMonth.toEnum(m$p);
      if ($0.tag === "Just") { return $0._1; }
      $runtime.fail();
    })(),
    d$p

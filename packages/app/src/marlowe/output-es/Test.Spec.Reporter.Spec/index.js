...
const print = dictMonadState => dictMonadWriter => path => v => {
  if (v.tag === "PrintSuite") { return dictMonadWriter.MonadTell1().tell(Data$dString$dCodeUnits.fromCharArray(Data$dArray.replicate(path.length)(" ")) + v._1 + "\n"); }
  if (v.tag === "PrintTest") {
    if (v._2.tag === "Success") {
      return dictMonadWriter.MonadTell1().tell(Data$dString$dCodeUnits.fromCharArray(Data$dArray.replicate(path.length)(" ")) + Test$dSpec$dStyle.styled(Test$dSpec$dStyle.green)("✓︎ ") + Test$dSpec$dStyle.styled(Test$dSpec$dStyle.dim)(v._1) + (v._2._1 === "Fast"
        ? ""
    ...
    if (v._2.tag === "Failure") {
      ...
      }))(v1 => dictMonadWriter.MonadTell1().tell(Data$dString$dCodeUnits.fromCharArray(Data$dArray.replicate(path.length)(" ")) + Test$dSpec$dStyle.styled(Test$dSpec$dStyle.red)(Data$dShow.showIntImpl(v1.numFailures) + ") " + $0) + "\n"));
    }
    $runtime.fail();
  }
  if (v.tag === "PrintPending") {
    return dictMonadWriter.MonadTell1().tell(Data$dString$dCodeUnits.fromCharArray(Data$dArray.replicate(path.length)(" ")) + Test$dSpec$dStyle.styled(Test$dSpec$dStyle.cyan)("- " + v._1) + "\n");
  }
  $runtime.fail();
};
...
const specReporter = /* #__PURE__ */ Test$dSpec$dReporter$dBase.defaultReporter(initialState)(/* #__PURE__ */ Test$dSpec$dReporter$dBase.defaultUpdate({
  ...
  putRunningItems: b => a => ({runningItems: b, numFailures: a.numFailures}),
  ...
  update: v => {
    ...
    if (v.tag === "TestEnd") {
      ...
      const $2 = v._3;
      return bind($$get)(v1 => {
        const $3 = Data$dMap$dInternal.lookup(Data$dOrd.ordArray(Test$dSpec$dTree.pathItemOrd))($1)(v1.runningItems);

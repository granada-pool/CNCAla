...
const parse = (state1, more, lift1, $$throw, done) => more(v1 => URI$dPath$dSegment.parseSegmentNZNC(
  state1,
  more,
  lift1,
  $$throw,
...
const genericPathNoScheme = {to: x => x, from: x => x};
const showPathNoScheme = {
  show: /* #__PURE__ */ (() => {
    const $0 = Data$dShow$dGeneric.genericShowConstructor((() => {
      ...
      return {genericShowArgs: v => ["(Tuple (Segment.unsafeSegmentNZNCToString (NonEmptyString.unsafeFromString " + Data$dShow.showStringImpl(v._1) + ")) " + $0(v._2) + ")"]};
    })())({reflectSymbol: () => "PathNoScheme"});
    return x => $0["genericShow'"](x);
  })()
};
const eqPathNoScheme = {eq: x => y => eq(x)(y)};
const ordPathNoScheme = {compare: x => y => compare(x)(y), Eq0: () => eqPathNoScheme};
export {PathNoScheme, compare, eq, eqPathNoScheme, fromFoldable, genericPathNoScheme, manyRec, ordPathNoScheme, parse, print, showPathNoScheme};

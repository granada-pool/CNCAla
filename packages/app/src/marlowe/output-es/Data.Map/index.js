...
const semigroupSemigroupMap = dictOrd => dictSemigroup => {
  ...
  return {append: v => v1 => Data$dMap$dInternal.unionWith(dictOrd)(append)(v)(v1)};
};
...
const monoidSemigroupMap = dictOrd => dictSemigroup => {
  const append = dictSemigroup.append;
  const semigroupSemigroupMap2 = {append: v => v1 => Data$dMap$dInternal.unionWith(dictOrd)(append)(v)(v1)};
  return {mempty: Data$dMap$dInternal.Leaf, Semigroup0: () => semigroupSemigroupMap2};
};
const keys = x => Data$dMap$dInternal.functorMap.map(v => {})(x);

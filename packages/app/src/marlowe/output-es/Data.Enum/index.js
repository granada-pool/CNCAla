...
const enumTuple = dictEnum => {
  ...
  return dictBoundedEnum => {
    ...
    return {
      succ: v => {
        const $0 = dictEnum.succ(v._1);
        const $1 = $0.tag === "Just" ? Data$dMaybe.$Maybe("Just", Data$dTuple.$Tuple($0._1, bottom2)) : Data$dMaybe.Nothing;
        const $2 = Data$dTuple.Tuple(v._1);
        const $3 = Enum1.succ(v._2);
        if ($3.tag === "Nothing") { return $1; }
        if ($3.tag === "Just") { return Data$dMaybe.$Maybe("Just", $2($3._1)); }
        $runtime.fail();
      },
      pred: v => {
        const $0 = dictEnum.pred(v._1);
        const $1 = $0.tag === "Just" ? Data$dMaybe.$Maybe("Just", Data$dTuple.$Tuple($0._1, top2)) : Data$dMaybe.Nothing;
        const $2 = Data$dTuple.Tuple(v._1);
        const $3 = Enum1.pred(v._2);
        if ($3.tag === "Nothing") { return $1; }
        if ($3.tag === "Just") { return Data$dMaybe.$Maybe("Just", $2($3._1)); }
        $runtime.fail();
      },
  ...
  return {
    succ: v => {
      ...
      if (v.tag === "Just") {
        ...
        if ($0.tag === "Just") { return Data$dMaybe.$Maybe("Just", Data$dMaybe.$Maybe("Just", $0._1)); }
...
const enumFromTo = dictEnum => {
  ...
  return dictUnfoldable1 => v => v1 => {
    if (Ord0.Eq0().eq(v)(v1)) { return Data$dUnfoldable1.replicate1(dictUnfoldable1)(1)(v); }

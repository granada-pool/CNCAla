...
const tailRecM2 = f => a => b => Control$dMonad$dRec$dClass.monadRecMaybe.tailRecM(o => f(o.a)(o.b))({a, b});
...
const unsnoc = lst => {
  ...
  if ($0.tag === "Just") { return Data$dMaybe.$Maybe("Just", {init: reverse($0._1.revInit), last: $0._1.last}); }
  return Data$dMaybe.Nothing;
};
...
const manyRec = dictMonadRec => dictAlternative => {
  ...
  return p => dictMonadRec.tailRecM(acc => dictMonadRec.Monad0().Bind1().bind(Alt0.alt(Alt0.Functor0().map(Control$dMonad$dRec$dClass.Loop)(p))($0.pure(Control$dMonad$dRec$dClass.$Step(
    ...
    if (aa.tag === "Done") { return Control$dMonad$dRec$dClass.$Step("Done", reverse(acc)); }
    $runtime.fail();
...
const nubByEq = v => v1 => {
  ...
  if (v1.tag === "Cons") {
    const $0 = v1._1;
    return Data$dList$dTypes.$List("Cons", $0, nubByEq(v)(filter(y => !v($0)(y))(v1._2)));
  }
  $runtime.fail();
};

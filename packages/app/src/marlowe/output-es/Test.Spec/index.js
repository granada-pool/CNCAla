...
const parallel = dictMonad => mapSpecTree(dictMonad.Bind1().Apply0().Functor0())(identity)(Test$dSpec$dTree.bimapTree(v => identity)(v => i => (
  {isParallelizable: i.isParallelizable.tag === "Nothing" ? Data$dMaybe.$Maybe("Just", true) : i.isParallelizable, example: i.example, isFocused: i.isFocused}
)));
const sequential = dictMonad => mapSpecTree(dictMonad.Bind1().Apply0().Functor0())(identity)(Test$dSpec$dTree.bimapTree(v => identity)(v => i => (
  {isParallelizable: i.isParallelizable.tag === "Nothing" ? Data$dMaybe.$Maybe("Just", false) : i.isParallelizable, example: i.example, isFocused: i.isFocused}
)));
const hoistSpec = dictMonad => {
  const mapSpecTree1 = mapSpecTree(dictMonad.Bind1().Apply0().Functor0());
  return onM => f => mapSpecTree1(onM)(Test$dSpec$dTree.bimapTree(name => around$p => i => f($ComputationType("CleanUpWithContext", name))(around$p(i)))(name => item => (
    {
      example: g => g((() => {
...
const focus = () => dictMonad => dictMonad.Bind1().Apply0().Functor0().map(m => Data$dTuple.$Tuple(
  m._1,
  any(Test$dSpec$dTree.treeFoldable.foldMap((() => {
    const semigroupDisj1 = {append: v => v1 => v || v1};
    return {mempty: false, Semigroup0: () => semigroupDisj1};
  })())(x => x.isFocused))(m._2)
    ? m._2
    : Data$dFunctor.arrayMap(Test$dSpec$dTree.bimapTree(v => identity)(v => v$1 => ({isFocused: true, isParallelizable: v$1.isParallelizable, example: v$1.example})))(m._2)
));
...
const aroundWith = dictMonad => {
  const mapSpecTree1 = mapSpecTree(dictMonad.Bind1().Apply0().Functor0());
  return action => mapSpecTree1(identity)(Test$dSpec$dTree.bimapTree(v => action)(v => v$1 => (
    {isFocused: v$1.isFocused, isParallelizable: v$1.isParallelizable, example: aroundAction => v$1.example(x => aroundAction(action(x)))}
  )));
};
